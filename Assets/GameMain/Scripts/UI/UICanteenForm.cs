...
namespace StarForce
{
    public class UICanteenForm : UGuiForm
    {
...
#endif
        {
            ...
            dataFood = GameEntry.Data.GetData<DataFood>();
        ...
        private void RefreshClothesStore()
        {
            ...
            {
                if (i >= dataFood.listCanteen.Count)
                {
                ...
                int id = dataFood.listCanteen[i];
                FoodData data = dataFood.GetFoodDataById(id);
                ...
                FoodDetail detail = foodSpriteList_SO.GetFoodDetail(data.IconId);
                item.sprite = detail.FoodIcon;
        ...
        public void OnSlotClick(GameObject obj)
        {
            ...
            if (0 <= curSlotIndex && curSlotIndex < dataFood.listCanteen.Count)
            {
        ...
        {
            if (curSlotIndex < 0 || curSlotIndex >= dataFood.listCanteen.Count)
            {
            ...
            int id = dataFood.listCanteen[curSlotIndex];
            FoodData data = dataFood.GetFoodDataById(id);
