    ...
    protected override void OnUpdate(ProcedureOwner fsm, float elapseSeconds, float realElapseSeconds)
    {
        ...
        if (logic.target)
            logic.transform.position = Vector2.MoveTowards(logic.transform.position,
                logic.target.position, logic.enemyData.ChaseSpeed * Time.deltaTime);
        ...
        if (Physics2D.OverlapCircle(logic.attackPoint.position, logic.attackArea, Constant.Layer.PlayerLayerId))
        {
